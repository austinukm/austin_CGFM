...
# ------------------------------------------------------------------------------
# 2. Model (HeavyTransformer Adapted)
# ------------------------------------------------------------------------------

class HeavyTransformerRecommender(nn.Module):
    def __init__(self, text_input_dim, img_input_dim, num_users, num_items, hidden_dim=256, num_heads=4, num_layers=2, temperature=0.07, user_emb_dim=64, proj_dim=256):
        ...
        # Projection to common space for recommendation (dot product with user)
        ...
        self.item_emb = nn.Embedding(num_items, proj_dim) # Optional item bias/embedding
    ...
    def forward_item(self, img_features, text_features, item_ids=None):
        ...
        # Mean pooling or take [CLS] equivalent? Original used mean(dim=1)
    ...
    def forward(self, user_ids, item_ids, img_features, text_features):
        ...
        # Return signature matching CGFM loop: scores, text, img, fused, gate(None)
...
def train_model(
    ...
    model = HeavyTransformerRecommender(
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train HeavyTransformer Model")
    ...
    parser.add_argument("--log-name", default="heavy_transformer_model")
