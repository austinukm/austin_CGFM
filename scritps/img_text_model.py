...
class MoiveDataset(torch.utils.data.Dataset):
    def __init__(self, csv_path: str, text_feature_path: str, img_feature_path: str):
        ...
        self.text_features = self._load_npz(text_feature_path)
        self.img_features = self._load_npz(img_feature_path)
        ...
        self.img_feature_tensor = torch.from_numpy(self.img_features["features"]).float()
    ...
    def construct_positive_dict(self) -> Dict[int, set]:
        positive_dict = {}
    ...
    def __construct_feature_dict(self, feature_npz: Dict[str, np.ndarray]) -> Dict[int, np.ndarray]:
        feature_dict = {}
        ids = feature_npz["ids"]
        ...
        for idx, mid in enumerate(ids):
            ...
            feature_dict[mid] = features[idx]
        return feature_dict

    def __len__(self) -> int:
        return len(self.data_df)

    def check_missing_features_get_allmovie_idx_map(self) -> Dict[int, int]:
        ...
        if set(text_keys) != set(img_keys):
            ...
            raise ValueError(
                f"AustinERROR Missing features found. Missing in text: {missing_in_text}, Missing in img: {missing_in_img}"
            )
        mid_idx_map = {}
    ...
    def get_user_id_dict(self) -> Dict[int, int]:
        user_ids = self.data_df["user_id"].unique().tolist()
        return {user_id: idx for idx, user_id in enumerate(user_ids)}

    def get_text_tensor_for_movie_ids(self, movie_ids: Sequence[int], device: Optional[torch.device] = None) -> Optional[torch.Tensor]:
        idxs = [self.all_movie_idx_map.get(mid) for mid in movie_ids if mid in self.all_movie_idx_map]
        if not idxs:
            return None
    ...
    def get_img_tensor_for_movie_ids(self, movie_ids: Sequence[int], device: Optional[torch.device] = None) -> Optional[torch.Tensor]:
        idxs = [self.all_movie_idx_map.get(mid) for mid in movie_ids if mid in self.all_movie_idx_map]
        if not idxs:
            return None
    ...
    def get_interact_dict(self, sample_num: int = 100) -> Dict[int, Dict[str, List[int]]]:
        ...
        for user_id, interacted_movies in interact_dict.items():
            non_interacted_movies = list(all_movie - interacted_movies)
            rng.shuffle(non_interacted_movies)
            interact_dict[user_id] = {
                "interact_movies": interacted_movies,
                "non_interact_movies": non_interacted_movies[: min(sample_num, len(non_interacted_movies))],
            }
...
class ImageTextModel(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.user_embeding = nn.Embedding(num_users, user_emb_dim)
        ...
        self.score_head = nn.Linear(proj_dim, 1)

    def forward(self, user_ids: torch.Tensor, text_features: torch.Tensor, img_features: torch.Tensor) -> torch.Tensor:
        user_emb = self.user_embeding(user_ids)
        user_proj = self.user_proj(user_emb)
        text_proj = self.text_proj(text_features)
        img_proj = self.img_proj(img_features)

        combined = torch.cat([user_proj, text_proj, img_proj], dim=-1)
...
def train_model(
    ...
    logger = _get_logger(log_name or "img_text_model")
    logger.info(
        "Model: ImageTextModel | text_dim: %d | img_dim: %d | proj_dim: %d | num_users: %d | dropout_rate: %.3f | epochs: %d | lr: %.6f",
        text_dim,
        img_dim,
        proj_dim,
        num_users,
    ...
    best_path = model_output_dir / "best_model_img_text.pth"
    ...
    for epoch in range(1, epoch_num + 1):
        ...
        for batch_idx, batch in enumerate(tqdm.tqdm(train_dl, desc=f"Training Epoch {epoch}")):
            ...
            scores = model(user_id_indices, text_features, img_features)
...
def compute_topk_metrics(model, val_ds, all_ds, ks=[5, 10, 20], sample_neg_num=100, max_users=None, logger=None):
    ...
    with torch.no_grad():
        for user_id in tqdm.tqdm(user_ids, desc="Computing Top-K metrics"):
            ...
            if text_feature_tensor is None or img_feature_tensor is None:
                if logger:
                    logger.debug(f"Skip user {user_id}: unable to fetch features.")
                continue
            ...
            scores = model(user_tensor, text_feature_tensor, img_feature_tensor).squeeze(-1)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train Image+Text multimodal model")
    ...
    parser.add_argument("--log-name", default="img_text_model")
