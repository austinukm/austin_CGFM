...
class MovieDataset(torch.utils.data.Dataset):
    def __init__(self, csv_path, text_feature_path, img_feature_path):
        ...
        # We load features just to get the ID maps consistent with CGFM
        # In MF, we don't actually use the features, but we need the same item indices
    ...
    def get_features_for_movie_ids(self, movie_ids, device=None):
        # For MF, we don't need features, but we need to return something to keep the interface consistent
        # or we just return None and handle it in evaluation
        return None, None
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train MF Model")
    ...
    parser.add_argument("--log-name", default="mf_model")
